// frontend/src/utils/tests/mysql.test.ts
...
describe("sql/mysql: generateMySQLSQL (current behavior)", () => {
  it("добавляет surrogate id (CHAR(36)) PK, если PK не задан (не link-таблица)", () => {
    const e = ent("u", "User", [attr("email", "TEXT")]);
    const sql = norm(generateMySQLSQL([e], []));
    expect(sql).toMatch(/CREATE TABLE `User`/);
    expect(sql).toMatch(/`id` CHAR\(36\) PRIMARY KEY/);
  });
  ...
  it("1:N: если FK нет — добавляет колонку + FK constraint + index", () => {
    ...
    const sql = norm(generateMySQLSQL([A, B], [r]));

    expect(sql).toMatch(/ALTER TABLE `Book`\n  ADD `author_id` CHAR\(36\) NOT NULL;/);
  ...
  it("1:N: если FK уже есть (authorId) — не добавляет колонку, а делает MODIFY NOT NULL", () => {
    const A = ent("a", "Author", [attr("id", "UUID", true)]);
  ...
  it("1:1: добавляет UNIQUE на FK и НЕ создаёт index (по умолчанию)", () => {
    ...
    const sql = norm(generateMySQLSQL([L, R], [rr]));

    expect(sql).toMatch(/ALTER TABLE `Right`\n  ADD `left_id` CHAR\(36\) NOT NULL;/);
  ...
  it("M:N (неявная): создаёт link-таблицу с PK(left,right) + 2 FK + индексы", () => {
    ...
    const sql = norm(generateMySQLSQL([U, R], [mm]));

    expect(sql).toMatch(/CREATE TABLE `user_role_link`/i);
    expect(sql).toMatch(/`user_id` CHAR\(36\) NOT NULL/);
    expect(sql).toMatch(/`role_id` CHAR\(36\) NOT NULL/);
  ...
  it("M:N (явная пустая link-таблица): текущая логика переименовывает в *_2 (uniqueName)", () => {
    ...
    const sql = norm(generateMySQLSQL([U, R, Link], [mm]));
  ...
  it("M:N (явная НЕпустая, compositePrimaryKey=false): текущая логика переименовывает в *_2 и добавляет UNIQUE(left,right)", () => {
    ...
    const sql = norm(generateMySQLSQL([P, T, Link], [mm]));

    expect(sql).toMatch(/CREATE TABLE `post_tag_2`/i);
    expect(sql).toMatch(/ALTER TABLE `post_tag_2`\n  ADD `post_id` CHAR\(36\) NOT NULL;/);
    expect(sql).toMatch(/ALTER TABLE `post_tag_2`\n  ADD `tag_id` CHAR\(36\) NOT NULL;/);
