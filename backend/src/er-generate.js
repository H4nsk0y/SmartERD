...
function normalizeSpec(spec) {
  ...
  for (const e of s.entities) {
    ...
    e.attributes = e.attributes.map((a) => ({
      ...
      primaryKey: !!a?.primaryKey || !!a?.isPrimaryKey,
      unique: !!a?.unique,
  ...
  for (const r of s.relationships) {
    ...
    if (r.link) {
      ...
      if (r.link.onDelete && !/^(CASCADE|SET NULL|RESTRICT|NO ACTION)$/i.test(r.link.onDelete))
        r.link.onDelete = undefined;
      if (r.link.onUpdate && !/^(CASCADE|SET NULL|RESTRICT|NO ACTION)$/i.test(r.link.onUpdate))
        r.link.onUpdate = undefined;
    ...
    if (r.fk) {
      ...
      if (r.fk.onDelete && !/^(CASCADE|SET NULL|RESTRICT|NO ACTION)$/i.test(r.fk.onDelete))
        r.fk.onDelete = undefined;
      if (r.fk.onUpdate && !/^(CASCADE|SET NULL|RESTRICT|NO ACTION)$/i.test(r.fk.onUpdate))
        r.fk.onUpdate = undefined;
...
function normalizeOneToXDirection(spec, maps) {
  for (const r of spec.relationships) {
    ...
    const expected = fkColName(fromE.name, fromPk.name);
    ...
    if (!r.fk) {
      r.fk = {
        column: expected,
        type: fromPk.type || "UUID",
        notNull: true,
        onDelete: "CASCADE",
        index: r.type === "one-to-many",
...
function removeMMIfOneToNExists(spec, aName, bName) {
  const hasOneToN = spec.relationships.some(
    (r) =>
      r.type === "one-to-many" &&
      ((eq(r.from, aName) && eq(r.to, bName)) || (eq(r.from, bName) && eq(r.to, aName)))
  );
  if (!hasOneToN) return;
  spec.relationships = spec.relationships.filter(
    (r) =>
      !(
        r.type === "many-to-many" &&
        ((eq(r.from, aName) && eq(r.to, bName)) || (eq(r.from, bName) && eq(r.to, aName)))
...
function autoRelationsFromIdColumns(spec, maps) {
  for (const e of spec.entities) {
    for (const a of e.attributes) {
      ...
      const exists = spec.relationships.some(
        (r) => r.type === "one-to-many" && eq(r.from, ref.name) && eq(r.to, e.name) && r?.fk?.column === a.name
      );
      if (!exists) {
...
function detectAndConvertLinkTables(spec, maps) {
  ...
  for (const c of linkCandidates) {
    ...
    spec.relationships = spec.relationships.filter(
      (r) => !(r.type !== "many-to-many" && (eq(r.from, c.e.name) || eq(r.to, c.e.name)))
    );

    const exists = spec.relationships.some(
      (r) =>
        r.type === "many-to-many" &&
        ((eq(r.from, c.leftRef.name) && eq(r.to, c.rightRef.name)) ||
          (eq(r.from, c.rightRef.name) && eq(r.to, c.leftRef.name))) &&
        r.link?.tableName &&
        eq(r.link.tableName, c.e.name)
    );
...
function fixPaymentRefund(spec, maps) {
  ...
  if (!has) {
    ...
    spec.relationships.push({
      ...
      to: refund.name,
      fk: {
        column: fkColName(payment.name, pk.name),
        type: pk.type || "UUID",
        unique: true,
        notNull: true,
        onDelete: "NO ACTION",
...
function fixUserEnrollment(spec, maps) {
  ...
  const exists = spec.relationships.some((r) => r.type === "one-to-many" && eq(r.from, user.name) && eq(r.to, enroll.name));
...
function preferOneToNOverMMForLessonComment(spec) {
  ...
  const hasOneToN = spec.relationships.some((r) => r.type === "one-to-many" && eq(r.from, lesson.name) && eq(r.to, comment.name));
  if (!hasOneToN) return;
  ...
  spec.relationships = spec.relationships.filter(
    (r) =>
      !(
        r.type === "many-to-many" &&
        ((eq(r.from, lesson.name) && eq(r.to, comment.name)) || (eq(r.from, comment.name) && eq(r.to, lesson.name)))
      )
  );
...
function stripWrongForeignAttrs(spec) {
  const pairFix = (parentName, childName, wrongFkOnParent) => {
    const hasCorrectRel = spec.relationships.some((r) => r.type === "one-to-many" && eq(r.from, parentName) && eq(r.to, childName));
...
function ensureSelfLinkByParentId(spec) {
  for (const e of spec.entities) {
    ...
    const hasSelf = spec.relationships.some(
      (r) => (r.type === "one-to-many" || r.type === "one-to-one") && eq(r.from, e.name) && eq(r.to, e.name)
    );
    if (hasSelf) continue;
...
export function repairSpec(inputSpec) {
  ...
  fixPaymentRefund(spec, maps);
  fixUserEnrollment(spec, maps);
  preferOneToNOverMMForLessonComment(spec);
